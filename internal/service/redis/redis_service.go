...
func SetKeyEx(key, value string, timeout time.Duration) error {
...
func GetKey(key string) (string, error) {
	...
	if err != nil {
		if errors.Is(err, redis.Nil) {
			zlog.Info("The key is not exist")
			return "", nil
		}
		return "", err
	}
...
func GetKeyWithPrefixNilIsErr(prefix string) (string, error) {
	...
	for {
		...
		if len(keys) == 0 {
			zlog.Info("No relevant prefix key found")
			return "", redis.Nil
		}

		if len(keys) == 1 {
			zlog.Info(fmt.Sprintln("Found relevant prefix key success", keys))
			return keys[0], nil
		} else {
			zlog.Error("找到了数量大于1的 key，查找异常")
			return "", errors.New("找到了数量大于1的 key，查找异常")
		}
	}
}
...
func GetKeyWithSuffixNilIsErr(suffix string) (string, error) {
	...
	for {
		...
		if len(keys) == 0 {
			zlog.Info("没有找到相关后缀的 key")
			return "", redis.Nil
		}

		if len(keys) == 1 {
			zlog.Info(fmt.Sprintln("成功找到了相关后缀的 key", keys))
			return keys[0], nil
		} else {
			zlog.Error("找到了数量大于1的 key，查找异常")
			return "", errors.New("找到了数量大于1的 key，查找异常")
...
func DelKeysWithPattern(pattern string) error {
	...
	for {
		...
		// 如果没有更多的键，则跳出循环
		if len(keys) == 0 {
			log.Println("No more keys found")
			break
		}
		// 删除找到的键
		if len(keys) > 0 {
			...
			log.Println("成功删除相关对应的 key", keys)
		}
	}
	return nil
}
...
func DelKeysWithPrefix(prefix string) error {
	...
	for {
		...
		if len(keys) == 0 {
			log.Println("No more prefix relevant keys found")
			break
		}
		if len(keys) > 0 {
			_, err = redisClient.Del(ctx, keys...).Result()
...
func DelKeysWithSuffix(suffix string) error {
	...
	for {
		...
		if len(keys) == 0 {
			log.Println("No more suffix relevant keys found")
			break
		}
		if len(keys) > 0 {
			_, err = redisClient.Del(ctx, keys...).Result()
